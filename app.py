...
# movie_id = st.text_input("Enter Genre (e.g., Action, Comedy)")
# n_recommend = st.slider("Number of recommendations", 1, 10, 5)

# # Recommendation Logic (Example)
# if st.button("Get Recommendations"):
#     st.write(f"Recommended movies for user {user_id}:")
#     st.write(f"Genre: {movie_id}")
#     recommended_movies = df.sample(n_recommend)
#     st.write(recommended_movies[['title', 'genre']])
...
# st.write("### Dataset Sample", movies_df.head())

# # Inputs
# user_id = st.number_input("Enter User ID", min_value=1, value=1)
# genre = st.text_input("Enter Genre (e.g., Action, Comedy)", value="Action")
# n_recommend = st.number_input("Number of Recommendations", min_value=1, max_value=10, value=5)
...
#     else:
#         st.write(f"Recommended movies for user {user_id} in {genre}:")
#         recommended_movies = filtered_movies.sample(n=min(n_recommend, len(filtered_movies)))
#         st.write(recommended_movies[['movie_id', 'title', 'genre']])
import streamlit as st
import pandas as pd
import numpy as np
from sklearn.decomposition import TruncatedSVD
